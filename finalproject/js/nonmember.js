	...
	function showTracks(genreID,artistID,albumID) {
		...
		//	content += '<h3>Tracks <a  data-genreid='+genreID+' data-artistid='+artistID+' data-albumid='+albumID+'  id="plustrack"><i class="icon-plus"></i></a></h3>';
			$.each(music[genreID].artists[artistID].albums[albumID].tracks,function(index,value) {
				...
				content += index;
				content += '><p contenteditable class="span4 trackname">';
				content += value.track;
				content += '</p>';
				content += '<p uneditable.textarea class="span3 trackfile">';				
				content += '<a class="ex2" href='+music[genreID].artists[artistID].albums[albumID].tracks[index].url+'</a>';
				//reserved for later use: content += value.url;
				content += '</p>';
				content += '<span class="label span1 ">Update</span><span class="label label-important span1 ">Delete</span></article>';
				});	
			content += "</section>";
			$("#track").html(content);
